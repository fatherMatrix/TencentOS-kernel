...
do {									\
	...
	__asm__ __volatile__(						\
		...
		"	jmp 2b\n"					\
		".previous\n"						\
		/* 下面三行在__ex_table(exception table)段中添加三个条目，每个
 		 * 条目中有两个关键字段：
 		 *    （发生缺页异常的地址，此地址的异常处理函数地址，...）
 		 * 其中的"发生缺页异常的地址"是上边负责拷贝的地址，"此地址的异
 		 * 常处理函数地址"是上边的fixup段的地址。
 		 *
 		 * fixup和__ex_table段的存在并不是直接替换到标准的缺页异常处理
 		 * 流程（针对可以恢复的情况，比如页被换出到磁盘），而是作为最后
 		 * 的处理手段（比如这个页确实没有对应的页表项）来防止内核crash:
 		 * o 对于用户态地址所在的页此时被换出到磁盘上，执行对应的处理函
 		 *   数将其换入，然后退出缺页异常处理函数，继续执行相应的汇编指
 		 *   令
 		 * o 如果标准的处理函数失败，则意味着这个缺页异常对应的页根本就
 		 *   没有对应的页表项，即这是个完全的非法地址。此时执行fixup段
 		 *   中的代码，返回此时还未拷贝的字节数
 		 *   如果没有fixup段做为最后的手段应对这个情况，内核会直接crash
 		 *   掉。也就是说，随便一个用户态进程都可以使用非法参数调用使用
 		 *   了copy_from_user的系统调用来crash掉内核，这太简单了，不应
 		 *   该发生
 		 */
